...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <!-- Member tidak memiliki akses edit/delete -->
</div>

<!-- Back to list button -->
<a href="{{ route('member.activities.index') }}" class="btn btn-outline-secondary btn-sm mb-3">
                                    ...
                                    <div class="reply-container">
                                        @foreach($comment->replies as $reply)
                                            <div class="reply-item" id="reply-{{ $reply->id }}">
                                                <div class="d-flex">
                                                    <div class="flex-shrink-0">
                                                        @if($reply->member->getPhotoDocument())
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('=== MEMBER ACTIVITIES DEBUG ===');
    
    // Find and setup reply buttons
    const replyButtons = document.querySelectorAll('.reply-btn');
    console.log(`Found ${replyButtons.length} reply buttons`);
    
    replyButtons.forEach((button, index) => {
        const commentId = button.getAttribute('data-comment-id');
        console.log(`Setting up reply button for comment ${commentId}`);
        ...
        button.addEventListener('click', function(e) {
            ...
            if (replyForm) {
                ...
                console.log(`Reply form ${replyForm.classList.contains('d-none') ? 'hidden' : 'shown'}`);
            } else {
                console.error(`Reply form not found: #reply-form-${commentId}`);
            }
        });
    });
