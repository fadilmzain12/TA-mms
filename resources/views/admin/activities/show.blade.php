...
<a href="{{ route('activities.index') }}" class="btn btn-outline-secondary btn-sm mb-3">
    <i class="bi bi-arrow-left me-2"></i> Kembali ke Kelola Kegiatan
</a>
                    ...
                    <div class="comment-item" id="comment-{{ $comment->id }}">
                        <div class="d-flex">
                            <div class="flex-shrink-0">
                                @if($comment->member->getPhotoDocument())
                            ...
                            <div class="flex-grow-1 ms-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <!-- Admin always has access to edit/delete -->
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu shadow-sm">
                                            <li>
                                                <form action="{{ route('comments.toggle-visibility', $comment) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="dropdown-item {{ $comment->is_hidden ? 'text-success' : 'text-warning' }}">
                                                        <i class="bi {{ $comment->is_hidden ? 'bi-eye' : 'bi-eye-slash' }} me-2"></i> 
                                ...
                                <div class="comment-content {{ $comment->is_hidden ? 'text-muted' : '' }}">
                                    @if($comment->is_hidden)
                                    <div class="alert alert-warning py-1 px-2 mb-2" style="font-size: 0.8rem;">
                                        <i class="bi bi-eye-slash-fill me-1"></i> Komentar ini disembunyikan dari member
                                    </div>
                                    @endif
                                    <p class="mb-0">{{ $comment->content }}</p>
                                </div>
                                
                                <!-- Edit form and other comment content remains the same as the original -->
                                <!-- ... (rest of comment structure) ... -->
                            </div>
                        </div>
                    </div>
                @empty
